...
print("--- Initial Concepts Count ---")
count = db.query(Concept).filter_by(project_id=material.project_id).count()
print(f"Concepts: {count}")

print("--- Running process_material (First pass) ---")
process_material(db, str(material.id))

print("--- After First Pass Concepts Count ---")
count = db.query(Concept).filter_by(project_id=material.project_id).count()
print(f"Concepts: {count}")
...
print("--- After Second Pass Concepts Count ---")
count2 = db.query(Concept).filter_by(project_id=material.project_id).count()
print(f"Concepts: {count2} (Diff: {count2 - count})")
